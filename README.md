...
- **The philosopher theme is rendered, not committed.** The sync is now
  theme-aware: when your instance is themed, it materialises a temporary copy of
  upstream's crew, applies your theme to it, and compares against the result. A
  themed instance exactly in step with upstream now sees an empty plan, and
  `python scripts/apply_theme.py philosophers` afterwards is a no-op. The
  framework still ships functional names; the theme is a local render step. The
  first sync carrying this fix still reports the old noise because `apply_theme.py`,
  `_crew_common.py`, and `_theme_common.py` are part of the synced `scripts/`
  allowlist — as described in the next bullet about how allowlist changes take
  two syncs.
