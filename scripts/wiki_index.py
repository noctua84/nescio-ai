...
from _marker_block import MarkerBlock, fenced_line_indices, split_lines
...
# classification; everything below owns what goes *inside* the block.
...
def outside_block(text: str) -> str:
    ...
    Only valid once `_BLOCK.classify` has returned `ok`.
    """
    return _BLOCK.outside(text)
...
def regenerate(dir_path: Path, *, check: bool = False) -> tuple[int, list[str]]:
    ...
    kind, _ = _BLOCK.classify(existing or "")
